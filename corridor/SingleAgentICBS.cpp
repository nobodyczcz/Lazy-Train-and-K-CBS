...
bool SingleAgentICBS::findPath(std::vector<PathEntry> &path, double f_weight, ConstraintTable& constraint_table, const bool* res_table, size_t max_plan_len, double lowerbound)
{
	...
	while (!focal_list.empty()) 
	{
		...
		if (curr->timestep >= constraint_table.length_max)
			continue;
		// if(curr->parent != NULL)
		// 	std::cout << "Parent loc: " << curr->parent->loc << " heading: " << -1 << " f: " << curr->parent->getFVal() << " g: " << curr->parent->g_val << " h: "<< curr->parent->h_val<<" num_internal_conf: " << curr->parent->num_internal_conf << " current lower boundary: "<< lower_bound << std::endl;

		// std::cout << "current loc: " << curr->loc << " heading: " << -1 <<" f: "<<curr->getFVal() << " g: " << curr->g_val << " h: " << curr->h_val << " num_internal_conf: " <<curr->num_internal_conf << " current lower boundary: " << lower_bound << std::endl;
		...
		for (int i = 0; i < 5; i++)
		{
			...
			if (validMove(curr->loc, next_id) && !my_map[next_id] && 
				!constraint_table.is_constrained(next_id, next_timestep) &&
